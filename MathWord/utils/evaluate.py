...
def cal_score(outputs, nums, ans, eqns):
	...
	for i in range(len(outputs)):
		...
		if 'NONE' in op:
			...
			pred, aligned = ans_evaluator(op, num)
	...
	return corr, eqn, tot, disp_corr, aligned
...
def answer_to_json(aligned):
	final_ans = parse_expr(aligned, evaluate=True)
	
	data = {}
	data["equation"] = aligned
	data["answer"] = str(int(final_ans))  
