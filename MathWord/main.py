...
from config.args import build_parser
...
from evaluate import run_validation, estimate_confidence
...
def load_data(config, logger):
    ...
    if config.mode == 'train':
        ...
        '''Load Datasets'''
        train_set = TextDataset(data_path=config.data_path, dataset=config.dataset,
                                datatype='train', max_length=config.max_length, is_debug=config.debug)
        val_set = TextDataset(data_path=config.data_path, dataset=config.dataset, datatype='dev', max_length=config.max_length, 
        ...
        test_set = TextDataset(data_path=config.data_path, dataset=config.dataset,
...
def main():
    ...
    if config.full_cv:
        ...
        data_path = config.data_path + data_name + '/'
